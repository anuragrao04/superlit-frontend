                  ...
                  <TabsList>
                    {
                      testData["questions"][currentQuestionIndex]["exampleCases"].map((testCase: any, index: any) => (
                        <TabsTrigger key={index} value={`case${index + 1}`} className={testCase.userOutput == null ? "text-white" : testCase.userOutput.trim() == testCase.expectedOutput.trim() ? "text-green-500" : "text-red-800"}>
                          Case {index + 1}
                        </TabsTrigger>
                      ))
                    }
