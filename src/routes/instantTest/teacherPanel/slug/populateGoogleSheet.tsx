...
export default function PopulateSheet({ privateCode, setDialog, dialogRef }: { privateCode: string, setDialog: any, dialogRef: any }) {
  ...
  const handlePopulateSheet = async () => {
    ...
    try {
      const response = await fetch("/api/instanttest/populategooglesheet", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ privateCode, googleSheetLink: sheetLink }),
      })
